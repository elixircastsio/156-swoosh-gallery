defmodule TeacherWeb.AlbumController do
  ...
  alias Teacher.{Recordings, Mailer}
  alias Teacher.Recordings.Album
  alias Teacher.Mailer.{
    AlbumAddedNotifier,
    AlbumUpdatedNotifier,
    AlbumDeletedNotifier
  }
  ...
  def create(conn, %{"album" => album_params}) do
    case Recordings.create_album(album_params) do
      {:ok, album} ->
        album
        |> AlbumAddedNotifier.album_added()
        |> Mailer.deliver()
  ...
  def update(conn, %{"id" => id, "album" => album_params}) do
    ...
    case Recordings.update_album(album, album_params) do
      {:ok, album} ->
        album
        |> AlbumUpdatedNotifier.album_updated()
        |> Mailer.deliver()
  ...
  def delete(conn, %{"id" => id}) do
    album = Recordings.get_album!(id)
    {:ok, album} = Recordings.delete_album(album)

    album
    |> AlbumDeletedNotifier.album_deleted()
    |> Mailer.deliver()
